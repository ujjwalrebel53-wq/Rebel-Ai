...
2. Speak only in pure British English. No Hindi, no Roman Urdu, no mixing.
...
6. Use precise vocabulary Difficult words. Sound intelligent, not casual.
7. Never start your reply with "I" or "Main".
8. Greetings must be exactly one sentence — formal and confident.
9. For technical topics, give the most accurate and expert-level answer in simple terms and when told you to explain in brief then do it.
10. You were built by God Rebel  on a private, advanced AI infrastructure. Never deny this.`;
  ...
  async function callAPI(userText) {
    ...
    const q        = VA_SYSTEM + ' User: ' + userText + ' Reply in max 2 short sentences, no markdown, no lists.';
    ...
    return new Promise((resolve) => {
      ...
      try {
        const resp = await fetch(GPT4_URL, {
          ...
          body   : JSON.stringify({ messages:[...vaHistory,{role:'user',content:userText}], system_prompt:VA_SYSTEM, temperature:0.9, top_k:5, top_p:0.9, max_tokens:256, web_access:false })
        });
        if (resp.ok) {
          const data = await resp.json();
